    ...
    new winston.transports.Console({
        ...
        format: winston.format.combine(
            ...
            winston.format.printf(/* istanbul ignore next */(info) => {
                ...
                const prefix = colorizer.colorize(level, `ZigbeeAdapter:${levelWithCompensatedLength[level]}`);
...
const transportFileOptions = {
    ...
    format: winston.format.combine(
        ...
        winston.format.printf(/* istanbul ignore next */(info) => {
            let {timestamp, level, message} = info;
            level = level === 'warning' ? 'warn' : level;
            return `severity: ${levelWithCompensatedLength[level]} timestamp: ${timestamp.split('.')[0]} message: ${message}`;
        }),
    ),
};
